...
class Report(QWidget):
    ...
    def view(self):
        ...
        data = postgres.select(f"SELECT INV_ID, TEN_ID, CONCAT(PER_FNAME, ' ', PER_LNAME) AS PER_NAME, STA_TYPE_NAME, INV_AMOUNT, PAY_ID, INV_DATE FROM INVOICE INNER JOIN TENANT USING (TEN_ID) INNER JOIN PERSONAL USING (PER_ID) INNER JOIN STALL USING (TEN_ID) INNER JOIN STALL_TYPE USING (STA_TYPE_ID) WHERE INV_DATE BETWEEN '{dateFrom}' AND '{dateTo}' ORDER BY INV_ID;")
    ...
    def view7D(self):
        self.table.clearContents()
        data = postgres.select("SELECT INV_ID, TEN_ID, CONCAT(PER_FNAME, ' ', PER_LNAME) AS PER_NAME, STA_TYPE_NAME, INV_AMOUNT, PAY_ID, INV_DATE FROM INVOICE INNER JOIN TENANT USING (TEN_ID) INNER JOIN PERSONAL USING (PER_ID) INNER JOIN STALL USING (TEN_ID) INNER JOIN STALL_TYPE USING (STA_TYPE_ID) WHERE INV_DATE BETWEEN CURRENT_DATE - INTERVAL '7 DAY' AND CURRENT_DATE ORDER BY INV_ID;")
