...
class Payment(QWidget):
    ...
    def search(self):
        ...
        data = postgres.select(f"SELECT PAY_ID, CONCAT(PER_FNAME, ' ', PER_LNAME) AS PER_NAME, STA_TYPE_NAME, PAY_AMOUNT - PAY_AMOUNT_PAID AS BALANCE, PAY_STATUS, PAY_DUE_DATE FROM PAYMENT INNER JOIN STALL_TYPE USING (STA_TYPE_ID) INNER JOIN STALL USING (STA_ID) INNER JOIN TENANT USING (TEN_ID) INNER JOIN PERSON USING (PER_ID) WHERE TEN_STATUS != 'Removed' AND LOWER(CONCAT(PAY_ID,' ', PER_FNAME, ' ', PER_LNAME, ' ', STA_TYPE_NAME)) LIKE LOWER('%{search}%') AND PAY_STATUS != 'Paid' AND PAY_STATUS != 'Cancelled' ORDER BY PAY_ID;")
    ...
    def updateFields(self):
        ...
        res = postgres.select(f"SELECT TEN_ID, PAY_ID, CONCAT(PER_FNAME, ' ', PER_LNAME) AS PER_NAME, STA_TYPE_NAME, PAY_STATUS, PAY_AMOUNT, PAY_DUE_DATE, PAY_AMOUNT - PAY_AMOUNT_PAID AS BALANCE FROM PAYMENT INNER JOIN STALL_TYPE USING (STA_TYPE_ID) INNER JOIN STALL USING (STA_ID) INNER JOIN TENANT USING (TEN_ID) INNER JOIN PERSON USING (PER_ID) WHERE PAY_ID = {id};")
    ...
    def updatePaymentData(self):
        data = postgres.query("UPDATE PAYMENT SET PAY_STATUS = CASE WHEN PAY_DUE_DATE < CURRENT_DATE THEN 'Overdue' ELSE PAY_STATUS END WHERE PAY_STATUS != 'Paid' AND PAY_STATUS != 'Cancelled' RETURNING PAY_ID;")
        if data:
            pass
